{{-- Nieuw contact koppelen aan een klant. --}}
<x-layouts.app title="Contact toevoegen">
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            Contact toevoegen — {{ $klant->voornaam }} {{ $klant->achternaam }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-4 sm:p-6 text-gray-900">
                    ...
                    <form method="POST" action="{{ route('klant.contact.store', $klant) }}" class="space-y-4">
                        @csrf
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" name="voornaam" value="{{ old('voornaam') }}" class="{{ $f }}" required maxlength="100">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Achternaam <span class="text-red-500">*</span></label>
                                <input type="text" name="achternaam" value="{{ old('achternaam') }}" class="{{ $f }}" required maxlength="100">
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" name="relatie" value="{{ old('relatie') }}" class="{{ $f }}" maxlength="50" placeholder="bijv. partner, broer, vriend">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Telefoonnummer <span class="text-red-500">*</span></label>
                                <input type="text" name="telefoonnummer" value="{{ old('telefoonnummer') }}" class="{{ $f }}" required maxlength="15" inputmode="numeric">
                            </div>
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700">Email</label>
                            <input type="email" name="email" value="{{ old('email') }}" class="{{ $f }}" maxlength="100">
                        </div>
                        ...
                        <div class="flex items-center gap-2">
                            <input type="checkbox" name="is_primair" id="is_primair" value="1" {{ old('is_primair') ? 'checked' : '' }}
