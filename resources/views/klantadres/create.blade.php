{{-- Extra adres toevoegen voor bestaande klant (POST naar klant.adres.store). --}}
<x-layouts.app title="Adres toevoegen">
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            Adres toevoegen — {{ $klant->voornaam }} {{ $klant->achternaam }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-4 sm:p-6 text-gray-900">
                    ...
                    <form method="POST" action="{{ route('klant.adres.store', $klant) }}" class="space-y-4">
                        @csrf
                        ...
                        <div>
                            ...
                            <input type="text" name="straat" value="{{ old('straat') }}" class="{{ $f }}" required maxlength="150">
                        </div>

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Postcode <span class="text-red-500">*</span></label>
                                <input type="text" name="postcode" value="{{ old('postcode') }}" class="{{ $f }}" required maxlength="10">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Stad <span class="text-red-500">*</span></label>
                                <input type="text" name="stad" value="{{ old('stad') }}" class="{{ $f }}" required maxlength="100">
                            </div>
                        </div>
