{{-- Formulier: bestaande klant wijzigen + gekoppeld adres (updateOrCreate op eerste adres). --}}
<x-layouts.app title="Klant wijzigen">
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Klant wijzigen</h2>
    </x-slot>
        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-4 sm:p-6 text-gray-900">
                    ...
                    <form method="POST" action="{{ route('klant.update', $klant) }}" class="space-y-4">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" name="voornaam" value="{{ old('voornaam', $klant->voornaam) }}" class="{{ $f }}" required maxlength="100" autocomplete="given-name">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Achternaam <span class="text-red-500">*</span></label>
                                <input type="text" name="achternaam" value="{{ old('achternaam', $klant->achternaam) }}" class="{{ $f }}" required maxlength="100" autocomplete="family-name">
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" name="telefoonnummer" value="{{ old('telefoonnummer', $klant->telefoonnummer) }}" class="{{ $f }}" required maxlength="15" inputmode="numeric" autocomplete="tel">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Email <span class="text-red-500">*</span></label>
                                <input type="email" name="email" value="{{ old('email', $klant->email) }}" class="{{ $f }}" required maxlength="100" autocomplete="email">
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            <div>
                                ...
                                <input type="number" name="aantal_volwassenen" value="{{ old('aantal_volwassenen', $klant->aantal_volwassenen) }}" class="{{ $f }}" min="0" required>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Aantal kinderen</label>
                                <input type="number" name="aantal_kinderen" value="{{ old('aantal_kinderen', $klant->aantal_kinderen) }}" class="{{ $f }}" min="0" required>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Aantal babys</label>
                                <input type="number" name="aantal_babys" value="{{ old('aantal_babys', $klant->aantal_babys) }}" class="{{ $f }}" min="0" required>
                            </div>
                        </div>
                        ...
                        <div class="border-t pt-4 mt-2">
                            ...
                            <div class="space-y-4">
                                <div>
                                    ...
                                    <input type="text" name="straat" value="{{ old('straat', $adres?->straat) }}" class="{{ $f }}" required maxlength="150" autocomplete="street-address">
                                </div>
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700">Postcode <span class="text-red-500">*</span></label>
                                        <input type="text" name="postcode" value="{{ old('postcode', $adres?->postcode) }}" class="{{ $f }}" required maxlength="10" autocomplete="postal-code">
                                    </div>
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700">Stad <span class="text-red-500">*</span></label>
                                        <input type="text" name="stad" value="{{ old('stad', $adres?->stad) }}" class="{{ $f }}" required maxlength="100" autocomplete="address-level2">
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <select name="IsActief" class="{{ $f }}">
                                <option value="1" {{ old('IsActief', $klant->IsActief ? '1' : '0') == '1' ? 'selected' : '' }}>Actief</option>
                                <option value="0" {{ old('IsActief', $klant->IsActief ? '1' : '0') == '0' ? 'selected' : '' }}>Inactief</option>
                            </select>
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700">Opmerking</label>
                            <textarea name="Opmerking" rows="2" class="{{ $f }}" maxlength="255">{{ old('Opmerking', $klant->Opmerking) }}</textarea>
                        </div>
